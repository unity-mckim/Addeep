...
test.describe("이벤트 CRUD 테스트", () => {
  test.beforeEach(async ({ page }) => {
    await login(page, TEST_USER);
    await switchTab(page, "이벤트");
  });

  test("이벤트 생성 페이지로 이동", async ({ page }) => {
    await clickCreateButton(page);

    // 생성 페이지 확인
    await expect(page).toHaveURL(/\/dashboard\/events\/create/);
    await expect(page.locator('h1:has-text("이벤트 만들기")')).toBeVisible();

    // 폼 필드 확인 - 이벤트는 더 많은 필드를 가질 수 있음
  ...
  test("이벤트 생성 - 빈 폼 제출 시 유효성 검사", async ({ page }) => {
    await clickCreateButton(page);
  ...
  test("이벤트 생성 - 취소 버튼 동작", async ({ page }) => {
    await clickCreateButton(page);
  ...
  test("이벤트 목록에서 항목 클릭하여 상세 페이지 이동", async ({ page }) => {
    ...
    if (rowCount > 0) {
      ...
      // 상세 페이지로 이동 확인
      await page.waitForURL(/\/dashboard\/events\/\d+/, { timeout: 5000 });

      // 상세 페이지 요소 확인
      await expect(page.locator("text=이벤트")).toBeVisible();
      await expect(page.locator('button:has-text("수정")')).toBeVisible();
      await expect(page.locator('button:has-text("삭제")')).toBeVisible();
    }
  });

  test("이벤트 상세 페이지 - 뒤로 가기 버튼", async ({ page }) => {
    await waitForLoadingComplete(page);
  ...
  test("이벤트 상세 페이지 - 수정 페이지로 이동", async ({ page }) => {
    await waitForLoadingComplete(page);
    ...
    if (rowCount > 0) {
      ...
      // 수정 페이지로 이동 확인
      await page.waitForURL(/\/dashboard\/events\/edit\/\d+/, {
        timeout: 5000,
      });
      await expect(
        page.locator('h1:has-text("이벤트 수정하기")')
      ).toBeVisible();
  ...
  test("이벤트 상세 페이지 - 삭제 모달 열기/닫기", async ({ page }) => {
    await waitForLoadingComplete(page);
  ...
  test("이벤트 수정 페이지 - 취소 버튼", async ({ page }) => {
    await waitForLoadingComplete(page);
    ...
    if (rowCount > 0) {
      ...
      // 상세 페이지로 돌아갔는지 확인
      await expect(page).toHaveURL(new RegExp(`/dashboard/events/${eventId}`));
    }
  });

  test("이벤트 테이블 렌더링", async ({ page }) => {
    await waitForLoadingComplete(page);
