...
test.describe("뉴스 CRUD 테스트", () => {
  test.beforeEach(async ({ page }) => {
    await login(page, TEST_USER);
    await switchTab(page, "뉴스");
  });

  test("뉴스 생성 페이지로 이동", async ({ page }) => {
    await clickCreateButton(page);

    // 생성 페이지 확인
    await expect(page).toHaveURL(/\/dashboard\/news\/create/);
    await expect(page.locator('h1:has-text("뉴스 만들기")')).toBeVisible();
  ...
  test("뉴스 생성 - 빈 폼 제출 시 유효성 검사", async ({ page }) => {
    await clickCreateButton(page);
  ...
  test("뉴스 생성 - 유효한 데이터로 생성", async ({ page }) => {
    await clickCreateButton(page);

    const testData = generateTestData("뉴스");

    // Alert 핸들러 설정
    setupAlertHandler(page, "데이터가 콘솔에 출력되었습니다!");

    // 폼 작성
    await page.fill('input[name="title"]', testData.title);
    await page.fill('textarea[name="content"]', testData.content);
    await page.fill('input[name="image"]', testData.image!);
  ...
  test("뉴스 생성 - 취소 버튼 동작", async ({ page }) => {
    await clickCreateButton(page);
  ...
  test("뉴스 목록에서 항목 클릭하여 상세 페이지 이동", async ({ page }) => {
    ...
    if (rowCount > 0) {
      ...
      // 상세 페이지로 이동 확인
      await page.waitForURL(/\/dashboard\/news\/\d+/, { timeout: 5000 });
      ...
      await expect(page.locator('button:has-text("뒤로 가기")')).toBeVisible();
    }
  });

  test("뉴스 상세 페이지 - 뒤로 가기 버튼", async ({ page }) => {
    await waitForLoadingComplete(page);
  ...
  test("뉴스 상세 페이지 - 수정 페이지로 이동", async ({ page }) => {
    await waitForLoadingComplete(page);
    ...
    if (rowCount > 0) {
      ...
      // 수정 페이지로 이동 확인
      await page.waitForURL(/\/dashboard\/news\/edit\/\d+/, { timeout: 5000 });
      await expect(page.locator('h1:has-text("뉴스 수정하기")')).toBeVisible();
  ...
  test("뉴스 상세 페이지 - 삭제 모달 열기/닫기", async ({ page }) => {
    await waitForLoadingComplete(page);
  ...
  test("뉴스 수정 페이지 - 폼 로드 확인", async ({ page }) => {
    await waitForLoadingComplete(page);
    ...
    if (rowCount > 0) {
      ...
      // 수정 버튼 클릭
      await page.click('button:has-text("수정")');
      await page.waitForURL(/\/dashboard\/news\/edit\/\d+/);
  ...
  test("뉴스 수정 페이지 - 취소 버튼", async ({ page }) => {
    await waitForLoadingComplete(page);
    ...
    if (rowCount > 0) {
      ...
      // 상세 페이지로 돌아갔는지 확인
      await expect(page).toHaveURL(new RegExp(`/dashboard/news/${newsId}`));
    }
  });

  test("뉴스 테이블 정렬 및 필터링", async ({ page }) => {
    await waitForLoadingComplete(page);
    ...
    if (rowCount > 0) {
      ...
      // 헤더가 클릭 가능한지 확인 (정렬)
      const titleHeader = page.locator('th:has-text("제목")');
